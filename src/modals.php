    ...
    <div class="modal-content" onclick="event.stopPropagation()" style="max-width: 700px;">
        <div class="panel-header-container">
            <h2 class="panel-header-title">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
        ...
        <form action="api.php?action=add_task" method="POST">
            <div class="form-grid">
                ...
                <div class="form-group"><label>Type / Couleur</label><select name="couleur"><option value="color-yellow">🟨 Standard</option><option value="color-blue">🟦 Étude/Tech</option><option value="color-orange">🟧 Urgence</option><option value="color-pink">🟥 Bug/Bloquant</option><option value="color-green">🟩 Validé</option><option value="color-grey">⬜ En attente</option></select></div>
                <div class="form-group"><label>Projet *</label><select name="projet" required><option value="">-- Sélectionner --</option><?php foreach($settings['projets'] as $p): ?><option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Code Projet</label><input type="text" name="code_projet" placeholder="Ex: PRJ-2026"></div>
                <div class="form-group"><label>Code ITBM</label><input type="text" name="code_itbm" placeholder="Ex: TSK0123456"></div>
                <div class="form-group"><label>Acteur / Porteur</label><select name="acteur"><option value="">-- Non assigné --</option><?php foreach($settings['acteurs'] as $a): ?><option value="<?= htmlspecialchars($a) ?>"><?= htmlspecialchars($a) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Priorité</label><select name="prio"><option value="">-- Non définie --</option><?php foreach($settings['priorites'] as $p): ?><option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Date de début</label><input type="date" name="date_debut"></div>
                <div class="form-group"><label>Échéance</label><input type="date" name="date_fin"></div>
                <div class="form-group full-width"><label>Note de suivi initiale (Optionnelle)</label><textarea name="note_initiale" rows="3" placeholder="Contexte initial..."></textarea></div>
            </div>
...
<div id="edit-task-modal" class="modal-overlay" onclick="closeEditTaskModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()" style="max-width: 700px;">
        <div class="panel-header-container">
            <h2 class="panel-header-title">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                Modifier la tâche
            </h2>
        ...
        <form action="api.php?action=edit_task" method="POST">
            ...
            <div class="form-grid">
                <div class="form-group full-width"><label>Intitulé de la tâche *</label><input type="text" name="titre" id="edit_titre" required></div>
                <div class="form-group"><label>Type / Couleur</label><select name="couleur" id="edit_couleur"><option value="color-yellow">🟨 Standard</option><option value="color-blue">🟦 Étude/Tech</option><option value="color-orange">🟧 Urgence</option><option value="color-pink">🟥 Bug/Bloquant</option><option value="color-green">🟩 Validé</option><option value="color-grey">⬜ En attente</option></select></div>
                <div class="form-group"><label>Projet *</label><select name="projet" id="edit_projet" required><option value="">-- Sélectionner --</option><?php foreach($settings['projets'] as $p): ?><option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Code Projet</label><input type="text" name="code_projet" id="edit_code_projet" placeholder="Ex: PRJ-2026"></div>
                <div class="form-group"><label>Code ITBM</label><input type="text" name="code_itbm" id="edit_code_itbm" placeholder="Ex: TSK0123456"></div>
                <div class="form-group"><label>Acteur / Porteur</label><select name="acteur" id="edit_acteur"><option value="">-- Non assigné --</option><?php foreach($settings['acteurs'] as $a): ?><option value="<?= htmlspecialchars($a) ?>"><?= htmlspecialchars($a) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Priorité</label><select name="prio" id="edit_prio"><option value="">-- Non définie --</option><?php foreach($settings['priorites'] as $p): ?><option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option><?php endforeach; ?></select></div>
                <div class="form-group"><label>Date de début</label><input type="date" name="date_debut" id="edit_date_debut"></div>
                <div class="form-group"><label>Échéance</label><input type="date" name="date_fin" id="edit_date_fin"></div>
            </div>
            <div style="text-align: right; border-top: 1px solid #dfe1e6; padding-top: 20px;">
...
<div id="notes-modal" class="modal-overlay" onclick="closeModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()">
        <div class="panel-header-container">
            <h2 class="panel-header-title">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
...
<div id="details-panel">
    <div class="panel-header-container">
        <h2 class="panel-header-title">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
            Détails & Suivi
        </h2>
