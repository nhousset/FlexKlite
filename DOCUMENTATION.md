...
- **Sécurité :** Définition du mot de passe Admin, et possibilité de forcer un mot de passe même pour la simple lecture de l'application (mode privé).
  ...
  <div align="center">
    <img src="src/img/help_premiere_connexion_admin.png" alt="Première connexion Admin" style="max-width: 100%; border-radius: 8px; margin: 15px 0; border: 1px solid #dfe1e6;">
  </div>

  > **🔑 Mot de passe Admin oublié ?** Si vous perdez le mot de passe administrateur, vous pouvez le réinitialiser manuellement. Allez dans le répertoire d'installation, ouvrez le fichier `src/db/admin.json` avec un éditeur de texte, et supprimez la ligne `"password": "..."` (ou supprimez simplement le fichier `admin.json`). En rechargeant la page de connexion, l'application vous demandera de créer un nouveau mot de passe.
